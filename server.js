...
app.prepare().then(() => {
  ...
  function contrast(e, f) {
    return e === f
  }
  server.get('/:id', (req, res) => {

    if (contrast('sheji', req.params.id)) {
      const actualPage = '/sheji'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (contrast('baojia', req.params.id)) {
      const actualPage = '/baojia'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (contrast('yanfang', req.params.id)) {
      const actualPage = '/yanfang'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (contrast('gywm', req.params.id)) {
      const actualPage = '/gywm'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (contrast('flsm', req.params.id)) {
      const actualPage = '/flsm'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (list[req.params.id]) {
      const actualPage = '/bkdetail'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (arr[req.params.id]) {
      const actualPage = '/shouye'
      const queryParams = {
        id: req.params.id
      }
      app.render(req, res, actualPage, queryParams)
    } else if (contrast('xgt', req.params.id)) {
  ...
  // 装修百科列表页
  server.get('/:id/:key', (req, res) => {
    if (list[req.params.id]) {
  ...
  server.get('/:id/[a-z]+', (req, res) => {
    if (contrast('xgt', req.params.id)) {
